...
test.describe("SauceDemo - Comprehensive Test Suite", () => {
  ...
  test.describe("Login Functionality", () => {
    ...
    for (const [key, user] of Object.entries(USERS)) {
      ...
      test(`should login successfully as ${key} user`, async ({
        page,
      }, testInfo) => {
  ...
  test.describe("Checkout Flow - Standard User", () => {
    ...
    test("should complete full checkout flow", async ({ page }, testInfo) => {
      ...
      await page.screenshot({ path: screenshot, fullPage: true });
      testInfo.attach("checkout-overview", {
        path: screenshot,
        contentType: "image/png",
      });
      ...
      testInfo.attach("checkout-complete", {
  ...
  test.describe("User Type Comparisons", () => {
    test("performance_glitch_user should be slower than standard_user", async ({
      page,
    }) => {
      ...
      console.log(
        `Standard: ${standardDuration}ms, Glitch: ${glitchDuration}ms`
      );
    });
  ...
  test.describe("Complete E2E Scenarios", () => {
    test("E2E: Browse, Sort, Add Multiple Items, Complete Purchase", async ({
      page,
    }, testInfo) => {
      // Login
      await page.locator("#user-name").fill(USERS.standard.username);
